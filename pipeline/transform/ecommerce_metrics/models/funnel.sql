...
SELECT * FROM (
  SELECT 1 AS step, 'Product View' AS step_name, s1 AS sessions, 100.0 AS percentage FROM counts UNION ALL
  SELECT 2 AS step, 'Add to Cart' AS step_name, s2 AS sessions, ROUND(100.0 * s2 / s1, 1) AS percentage FROM counts UNION ALL
  SELECT 3 AS step, 'Purchase' AS step_name, s3 AS sessions, ROUND(100.0 * s3 / s1, 1) AS percentage FROM counts
)
ORDER BY step